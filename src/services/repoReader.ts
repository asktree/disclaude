...
export class RepoReader {
  ...
  async getRelevantFiles(query: string): Promise<{ path: string; content: string }[]> {
    ...
    return files.filter(
      (f) => !f.content.startsWith("File not found") && !f.content.startsWith("Unable to fetch"),
    );
  }
  ...
  async getRelevantFilesByTopics(topics: string[]): Promise<{ path: string; content: string }[]> {
    ...
    const topicFileMap: Record<string, string[]> = {
      ...
      "web-search": ["src/services/webSearch.ts", "src/handlers/messageHandler.ts"],
      images: ["src/services/claude.ts"],
      "url-fetching": ["src/services/urlFetcher.ts"],
