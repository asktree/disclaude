...
- Posts in another language are shown translated, with a small "Translated from …" note (the original is behind the link). `TWEET_TRANSLATE_TO` sets the target language (default `en`; set it to an empty string to disable). Quoted posts are not translated.
